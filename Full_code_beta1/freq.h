...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	else   // If the first rising edge is captured, now we will capture the second edge
	{
		...
		frequencyTorque = (10000-frequencyTorque)/1000; //frequency to torque conversion formula
...
void outputTorque()
{
	if(frequencyTorque != 0)
	{
		sprintf((char*)frequencyCapture, "%f", frequencyTorque); //converts frequencyTorque into a character string
		HAL_UART_Transmit(&huart1, frequencyCapture, sizeof(frequencyCapture), 1000);
		HAL_UART_Transmit(&huart1, n1, sizeof(n1), 1000);
	}
}
