...
//Chip select GPIO
#define CHIP_SELECT GPIO_PIN_12
#define CSPORT GPIOB
//SPI reset GPIO
...
void resetDisplay()
{
	HAL_GPIO_WritePin(RSTPORT, RESET, GPIO_PIN_RESET);
	HAL_Delay(300); //hold low for 0.3 seconds
	HAL_GPIO_WritePin(RSTPORT, RESET, GPIO_PIN_SET);
...
{
	HAL_GPIO_WritePin(CSPORT, CHIP_SELECT, GPIO_PIN_RESET); //set CS low for transmission
	HAL_SPI_Transmit(&hspi2, data, 3, 1000); //send data to display
	HAL_GPIO_WritePin(CSPORT, CHIP_SELECT, GPIO_PIN_SET); //set CS high to end transmission
}
...
void setTwoLineMode()
{
	uint8_t lineMode[3] = {0xF8, 0x30, 0xC0};
	sendSPI(lineMode);
}
...
void initializeDisplay()
{
	...
	setTwoLineMode(); //put display in 2 line mode
}
