...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
bool motorStatus = 0; //1 = motor on, 0 = motor off
/* USER CODE END 0 */
...
int main(void)
{
  ...
  //Set motor on 1-second 900 RPM ramp to indicate that test fixture has started
  setMotor30();
  HAL_Delay(1000);
  stopMotor();
  setMotorSpeed(0);
	  ...
	  if(HAL_UART_Receive(&huart1, &rxdata, 1, 1000) == HAL_OK) //receive from UART and store in rx buffer
	  {
		  switch(rxdata){
		  	  ...
		  	  case '+':
				  if(motorStatus == 1)
				  {
					  if (spd_freq < 1300){ //increase speed if max speed not reached
						  spd_freq += 5;
					  }
					  else{
					  ...
					  displayRPM(spd_freq); //display the set RPM on the LCD
				  }
				  break;
			  //motor speed (-) from keyboard input
		  	  case '-':
				  if(motorStatus == 1)
				  {
					  if (spd_freq > 0){ //decrease speed if speed != 0
						  spd_freq -= 5;
					  }
					  else{
						  spd_freq -= 0; //do nothing to the speed
			  ...
			  //start motor
		  	  case 'g':
				  startMotor();
				  spd_freq = 10;	//set 30 RPM on startup
				  setMotorSpeed(10);
				  HAL_UART_Transmit(&huart1, startmsg, sizeof(startmsg), 1000);
				  motorStatus = 1;
				  break;
			  //default condition 150 RPM
			  default:
				  spd_freq = 50;
				  setMotorSpeed(spd_freq);
				  break;
