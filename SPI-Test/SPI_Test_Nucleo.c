...
void resetDisplay()
{
	//reset the display (set low to reset)
	//RST is on PC5
...
{
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); //set CS low for transmission
	HAL_SPI_Transmit(&hspi1, data, 3, 1000); //send data to display
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); //set CS high to end transmission
}
...
void initializeDisplay()
{
	...
	HAL_SPI_Transmit(&hspi1, dummyData, 1, 1000); //send data to display
...
int main(void)
{
  ...
  MX_SPI1_Init();
  /* USER CODE BEGIN 2 */
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); //set CS high for default state
