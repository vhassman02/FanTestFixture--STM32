...
int main(void)
{
  ...
  HAL_UART_Transmit(&huart1, hellomsg, sizeof(hellomsg), 1000); //transmit a "Hello" string on UART1_TX
  ...
  uint8_t one[1] = {0xC8};
  uint8_t two[1] = {0xC4};
  uint8_t three[1] = {0xCC};
  uint8_t four[1] = {0xC2};
  uint8_t five[1] = {0xCA};
  uint8_t six[1] = {0xC6};

  int counter = 0;
  uint16_t spd_freq = 0; //holds the current speed as a 16-bit integer
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  /*SPI CODE*/
	  //Handles writing characters to the LCD display for testing purposes
	  sendSPIData(one);
	  sendSPIData(two);
	  sendSPIData(three);
	  sendSPIData(four);
	  ...
	  /*MODBUS CODE*/
	  //Handles the send/receive of USB commands and RS485 MODBUS motor commands
	  if(HAL_UART_Receive(&huart1, &rxdata, 1, 1000) == HAL_OK) //receive from UART and store in rx buffer
	  {
		  switch(rxdata) {
		  case '+': //motor speed + from keyboard input
			  if (spd_freq < 1370) //only increase speed if max speed not reached
				  spd_freq+=5;
			  else
				  spd_freq += 0; //do nothing to the speed
			  setMotorSpeed(spd_freq);
			  break;
		  case '-': //motor speed - from keyboard input
			  if (spd_freq > 0) //only decrease speed if speed != 0
				  spd_freq-=5;
			  else
				  spd_freq -= 0; //do nothing to the speed
			  setMotorSpeed(spd_freq);
		  ...
		  case 'g': //start motor
			  startMotor();
			  setMotorSpeed(10); //set base RPM whenever motor is started
			  spd_freq = 10;	//update speed variable
